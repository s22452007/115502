...
def subscribe():
    ...
    # 將所有現有訂閱的 auto_renew 設為 False（標為取消）
    ...
    # 移除所有排程升級（pending）
    pending_subs = UserSubscription.query.filter(
...
def schedule_upgrade():
    ...
    current_sub = (UserSubscription.query
                   .filter(UserSubscription.user_id == user_id,
                           UserSubscription.end_date >= now_dt,
                           UserSubscription.auto_renew == True,
                           UserSubscription.start_date <= now_dt)
                   .order_by(UserSubscription.created_at.desc())
                   .first())
    if not current_sub:
        return jsonify({'error': '找不到有效訂閱'}), 404
...
def pay_pending():
    ...
    pending = UserSubscription.query.filter(
        UserSubscription.user_id == user_id,
