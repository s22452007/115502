...
def get_profile_data(user_id):
    ...
    scene_count = db.session.query(func.count(func.distinct(Vocab.scene_id))).select_from(UserVocab).join(Vocab, UserVocab.vocab_id == Vocab.id).filter(UserVocab.user_id == user_id).scalar() or 0
